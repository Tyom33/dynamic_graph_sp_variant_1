...
class FloydWarshall {
public:
    void floydWarshall(std::map<std::pair<int, int>, int>& shortestpaths, std::map<int, std::set<std::pair<int, int>>>& adjList, int a, int b) {
        ...
        if (it != shortestpaths.end()) {
            ...
            std::vector<std::vector<long int>> dist(n, std::vector<long int>(n, std::numeric_limits<int>::max()));
...
class Djikstra {
public:
    std::pair<long int, std::vector<int>> djikstra(std::map<std::pair<int, int>, int>& shortestpaths, std::map<int, std::set<std::pair<int, int>>>& adjList, int start, int end) {
        int n = adjList.rbegin()->first + 1;
        std::vector<long int> dist(n, std::numeric_limits<long int>::max());
        dist[start] = 0;
        std::vector<bool> visited(n, false);
        std::vector<std::vector<int>> vertexQueue(n);
        vertexQueue[start].push_back(start);
        for (int i = 0; i < n; i++) {
            int nearest = -1;
            for (int v = 0; v < n; v++) {
